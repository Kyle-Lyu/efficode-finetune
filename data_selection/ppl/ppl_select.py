...
def main():
    parser = argparse.ArgumentParser(description="Sample data by PPL from a JSONL file.")
    ...
    print(f"Loaded {len(scores)} PPL scores from {args.score_path}")

    # 将 PPL 分数数据转换为字典，方便根据 id 查找
    ...
    for item in data:
        ...
        if item_id in score_dict:
            item[PPL_KEY] = score_dict[item_id][PPL_KEY]
            merged_data.append(item)

    # 根据 PPL 分数从大到小排序
    sorted_data = sorted(merged_data, key=lambda x: x[PPL_KEY], reverse=True)
