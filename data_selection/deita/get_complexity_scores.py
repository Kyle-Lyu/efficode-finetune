...
def get_args():
    parser = argparse.ArgumentParser(description="Calculate complexity scores for instruction-response pairs.")
    ...
    parser.add_argument(
        ...
        help="Path to the pre-trained complexity scorer model."
    )
    parser.add_argument(
...
def main():
    ...
    for item in tqdm(data, desc=f"Processing on device {os.environ.get('CUDA_VISIBLE_DEVICES', None)}"):
        ...
        complexity_score = scorer.infer_complexity(inst)
